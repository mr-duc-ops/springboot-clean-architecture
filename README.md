...
![Uncle Bob's Clean Architecture in an image](README/IMAGES/cleanarc.png)
...
1. How to run the applications
   ...
   0. Optional to automation generate .mvn/wrapper/ including maven-wrapper.jar and maven-wrapper.properties:

   ```bash
   mvn -N io.takari:maven:wrapper -Dmaven=3.9.9
   ```

   1. To run `cleanarch-from-mr_duc` application, change to its directory and run
